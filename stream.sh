...
# while true; do
streamlink --loglevel warning --stdout "$URL" best | ffmpeg -loglevel warning -y -i pipe:0 -r "$FRAME_RATE" -t 24:00:00 -vf drawtext='fontfile=FreeSans.ttf:text=%{localtime\\:%b %d %Y %I %p}:box=1:boxcolor=#FFFFFF@0.5:boxborderw=5:x=5:y=5' "$OUTPUT_PATH"
# done
